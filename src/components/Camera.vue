<template>
  <div class="text-center background">
    <div class="controls d-flex justify-center">
      <v-btn v-if="!scanning" @click="startScanning" class="my-3 mx-2" color="card">Start Scanning</v-btn>
      <v-btn v-else @click="stopScanning" class="my-3 mx-2" color="card">Stop Scanning</v-btn>

      <div id="flash-control" v-if="hasFlash">
        <v-btn v-if="!flash" @click="turnOnFlash" class="my-3 mx-2" color="card">Turn on Flash</v-btn>
        <v-btn v-else @click="turnOffFlash" class="my-3 mx-2" color="card">Turn off flash</v-btn>
      </div>
    </div>
    <div v-if="checking">
      <p>Checking Code</p>
...
export default {
  ...
  methods: {
    ...
    turnOnFlash () {
      try {
        this.reader.toggleFlash()
        this.flash = true
      } catch (exception) {
        console.log('Flash did not work')
      }
    },
    turnOffFlash () {
      if (this.reader.hasFlash() && this.reader.isFlashOn()) {
        this.reader.turnFlashOff()
        this.flash = false
      }
    }
  },
  ...
  mounted () {
    ...
    this.reader = new QrScanner(this.$refs.scanner, this.onScanSuccess)
    this.reader.setCamera('environment')
    this.reader.start()
    this.reader.hasFlash().then((hasFlash) => {
